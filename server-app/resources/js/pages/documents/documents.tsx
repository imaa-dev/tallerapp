...
import { BreadcrumbItem, Pagination, RepairDocument } from '@/types';
import { Head } from '@inertiajs/react';
import { Calendar, Download, Eye, FileQuestion, FileText } from 'lucide-react';
...
export default function Documents({ documents, pagination: initialPagination }: DocumentsProps) {
    ...
    const renderDocument = (doc: RepairDocument | undefined, title: string) => {
        if (!doc) {
            return (
                <div className="rounded-xl border border-dashed bg-gray-50 p-6 dark:bg-gray-900">
                    <div className="flex h-full flex-col items-center justify-center text-center">
                        ...
                        <p className="mt-2 text-sm text-gray-500">Documento no disponible</p>
                    </div>
                </div>
            );
        }
            ...
            <div className="rounded-xl border bg-white p-6 shadow-sm transition hover:shadow-md dark:border-gray-700 dark:bg-gray-900">
                <div className="mb-5 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500">Servicio #{doc.service_id}</p>
                    </div>
                </div>

                <div className="space-y-3 text-sm">
                    <div>
                        <p className="font-medium text-gray-500">Archivo</p>
                ...
                <div className="border-sidebar-border/70 dark:border-sidebar-border relative min-h-[100vh] flex-1 overflow-hidden rounded-xl border md:min-h-min">
                    <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                        <div className="mb-6 flex items-center gap-3">
                            ...
                            <h1 className="text-2xl font-semibold">Documentos del servicio</h1>
                        </div>
                        ...
                        <div className="grid gap-6 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3">
                            {Array.from(groupedDocuments.entries()).map(([serviceId, docs]) => (
                                <div key={serviceId} className="col-span-full rounded-xl border p-6">
                        ...
                        <DataFilterPagination pagination={pagination} onPageChange={searchDocuments} />
                    </div>
                </div>
            </div>
        </AppLayout>
